...
func main() {
	...
	// Init file services
	fileService := storage.New(ctx, storage.FILE)(storage.DRIVE, &storage.Config{GoogleDrive: storage.GoogleDrive{
		PoolSize:     4,
		ByHTTPClient: false,
	...
	if err != nil {
		log.Fatalln("Unable to retrieve files: ", err)
	}
	...
	if err := fileService.Delete(fileIDs); err != nil {
		log.Fatalln("Can not delete: ", err)
	}
	...
	if err != nil {
		log.Fatalln("Can not create folder: ", err)
	}
	...
	if err != nil {
		log.Fatalln("Can not move file: ", err)
	}
	...
	if err != nil {
		log.Fatalln("Can not upload: ", err)
	}
